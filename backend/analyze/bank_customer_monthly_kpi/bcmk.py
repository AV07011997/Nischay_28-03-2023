...
def KPIs(data):
    try:
        ...
        ###########################Inflows
        #####Non Cash credits
        Non_cash_credits = final[~(final["mode"] == 'Cash')].groupby(['month_year']).agg(
            Non_cash_credits_Count=("credit", "count"), Non_cash_credits_Value=("credit", "sum")).reset_index()

        #####Cash credits
        Cash_credits = final[(final["mode"] == 'Cash')].groupby(['month_year']).agg(
            Cash_credits_Count=("credit", "count"), Cash_credits_Value=("credit", "sum")).reset_index()

        #####Total credits
        Total_credits = final.groupby(['month_year']).agg(Total_credits_Count=("credit", "count"),
                                                          Total_credits_Value=("credit", "sum")).reset_index()
        ...
        #####Cash debits
        Cash_debits = final[(final["mode"] == 'Cash')].groupby(['month_year']).agg(Cash_debits_Count=("debit", "count"),
                                                                                   Cash_debits_Value=(
                                                                                   "debit", "sum")).reset_index()

        #####Total debits
        Total_debits = final.groupby(['month_year']).agg(Total_debits_Count=("debit", "count"),
                                                         Total_debits_Value=("debit", "sum")).reset_index()

        ##### Auto debits
        Auto_debits = final[(final["mode"].isin(["Loan/MF", "MF", "Loan"]))].groupby(['month_year']).agg(
            Auto_debits_Count=("debit", "count"), Auto_debits_Value=("debit", "sum")).reset_index()
        ...
        #######Month_End_balance'
        # final.info()
        check_month_df = {"month_min": (final['txn_date'].min()).month, "month_max": (final['txn_date'].max()).month,
                          "year_min": (final['txn_date'].min()).year, "year_max": (final['txn_date'].max()).year}

        if ((check_month_df['month_min'] == check_month_df['month_max']) and (
                check_month_df['year_min'] == check_month_df['year_max'])):
            try:
                months_df = pd.DataFrame([{"months": final["txn_date"].max()}])
        ...
        while Month_End_balance1["Month_End_balance"].isnull().sum() > 0:
            Month_End_balance1["Month_End_balance"] = np.where(Month_End_balance1["Month_End_balance"].isnull(),
                                                               Month_End_balance1["Month_End_balance"].shift(1),
                                                               Month_End_balance1["Month_End_balance"])
        #####EMI
        ...
        months_12 = pd.DataFrame({"months": pd.date_range(
            (final["txn_date"].max() + relativedelta(day=31)) + relativedelta(months=-12),
            (final["txn_date"].max() + relativedelta(day=31)), freq='MS')})
        ...
        credits = months_12.merge(Non_cash_credits, on="month_year", how="left").merge(Cash_credits, on="month_year",
                                                                                       how="left").merge(Total_credits,
                                                                                                         on="month_year",
                                                                                                         how="left")

        debits = months_12.merge(Non_cash_debits, on="month_year", how="left").merge(Cash_debits, on="month_year",
                                                                                     how="left").merge(Total_debits,
                                                                                                       on="month_year",
                                                                                                       how="left").merge(
            Auto_debits, on="month_year", how="left")

        cd1 = credits.merge(debits, on="month_year", how="left").merge(Max_credit_Amount, on="month_year",
                                                                       how="left").merge(Max_debit_Amount,
                                                                                         on="month_year", how="left")
        cd2 = cd1.merge(Average_balance, on="month_year", how="left").merge(Month_End_balance1, on="month_year",
                                                                            how="left")
        # cd2.info()
        cd2.columns
        ...
        Monthly_KPIs.iloc[0, :] = Monthly_KPIs.iloc[0, :].astype(str).str.split("-").apply(
            lambda x: dt.date(int(x[0]), int(x[1]), 1))
