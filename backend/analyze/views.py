...
def bureau_customer_kpi(request):
    ...
    if "deal_id" not in request.session or "customer_id" not in request.session:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT * FROM a3_kit.bureau_account_segment_tl WHERE CUSTOMER_ID = " + customer_id + " AND final_selected=1 ;")
        ...
        def gantt_chart_loan_timeline(df):
            ...
            df['security_status'] = np.where(df['ACCOUNT_TYPE'].isin(['5', '6', '8', '9', '10', '12',
                                                                      ...
                                                                      41, 43, 44, 45, 51, 52,
                                                                      53, 54, 55, 56, 57, 58, 61, 00]), "unsecured",
                                             "secured")
            ...
            plotly.offline.plot(fig1,
                                filename=r"/Users/hardikbhardwaj/Downloads/a3_kit/templates/test1.html",
                                auto_open=False)
            ...
            plotly.offline.plot(fig2,
                                filename=r"/Users/hardikbhardwaj/Downloads/a3_kit/templates/test2.html",
                                auto_open=False)
        ...
        if type(KPI) is not type(None):
            ...
            with connection.cursor() as cursor:
                cursor.execute(
                    "SELECT count(CUSTOMER_ID) as No_Of_Address FROM bureau_address_segment WHERE CUSTOMER_ID = " + customer_id + " GROUP BY ADDRESS_1,ADDRESS_2,ADDRESS_3,ADDRESS_4,ADDRESS_5,source" + ";")
            ...
            return render(request, "bureau_c_k.html",
...
def bank_entity(request):
    ...
    with connection.cursor() as cursor:
        if entity == "Overall":
            cursor.execute(
                "SELECT * FROM bank_bank WHERE  customer_id=" + cid + " and account_number like " + accno + ";")
            data = dictfetchall(cursor)
            # for date in data:
        ...
        cursor.execute(
            "SELECT * FROM bank_bank WHERE customer_id=" + cid + " and account_number like " + accno + " and entity = '" + entity + "';")
        data = dictfetchall(cursor)
        data = pd.DataFrame(data)
        try:
            data['txn_date'] = data['txn_date'].apply(lambda x: x.strftime('%d/%m/%Y'))
...
#
#     if type == 'credit':
#         data1 = bank_bank.objects.filter(ammount__contains=accno).values(
#             'txn_date',
#             'credit',
#             'debit',
#             'balance',
...
#         )
#
#         # data = bank_bank.objects.filter(account_number=accno, credit=amount).values(
#         #     'txn_date',
#         #     'description',
#         #     'debit',
#         #     'credit',
...
#
#     if type == 'debit':
#         data1 = bank_bank.objects.filter(account_number__contains=accno).values(
#             'txn_date',
#             'credit',
#             'debit',
#             'balance',
...
#         data = bank_bank.objects.filter(account_number=accno, balance__lte=Decimal('0')).values('txn_date',
#                                                                                                           'description',
#                                                                                                           'debit',
#                                                                                                           'credit',
#                                                                                                           'balance')
...
#
#     if type == 'credit':
#         data = bank_bank.objects.all()
#         # data = bank_bank.objects.filter(credit=amount)
#         # data = bank_bank.objects.filter(account_number=accno).values('account_number', 'bank_name')
#         # data=bank_bank.objects.filter(account_number__contains=accno, credit=amount)
#
#     if type == 'debit':
#         with connection.cursor() as cursor:
#             cursor.execute(
...
def bck_popup(request):
    ...
    try:
        ...
        filtered_data['credit'] = filtered_data['credit'].apply(lambda x: format_currency(x, 'INR', locale='en_IN'))
        filtered_data['balance'] = filtered_data['balance'].apply(lambda x: format_currency(x, 'INR', locale='en_IN'))
        filtered_data['txn_date'] = filtered_data['txn_date'].apply(lambda x: x.strftime('%d/%m/%Y'))
    except:
        pass
