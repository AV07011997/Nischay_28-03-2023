...
data_basic = data_basic[['deal_id', 'cust_id', 'source',
                         'report_id', 'date_issued', 'DOB', 'name', 'PAN', 'SCORE']]
...
sql1 = "SELECT CUSTOMER_ID AS cust_id, BUREAU_ID AS report_id, ACCOUNT_HD_SEGMENT AS loan_id,ACCOUNT_TYPE AS account_type," \
       ...
       "source FROM a5_kit.mysite_bureau_account_segment_tl " \
...
sql1 = "SELECT CUSTOMER_ID AS cust_id, BUREAU_ID AS report_id, ENQUIRY_SEGMENT_HEADER AS inquiry_id,DATE_OF_ENQUIRY AS date_of_inquiry," \
       "ENQUIRY_PURPOSE AS purpose_of_inquiry, source FROM a5_kit.mysite_bureau_enquiry_segment_iq"

data_inquiry = pd.read_sql(sql1, conn)


data_inquiry = data_inquiry[['source', 'report_id', 'cust_id',
                             'inquiry_id', 'date_of_inquiry', 'purpose_of_inquiry']]
data_inquiry['date_of_inquiry'] = pd.to_datetime(
    data_inquiry['date_of_inquiry'])
...
sql1 = "SELECT CUSTOMER_ID AS cust_id, BUREAU_ID AS report_id, ADDRESS_1,ADDRESS_2,ADDRESS_3,ADDRESS_4,ADDRESS_5,DATE_OF_ADDR_REPORTED_TO_BUREAU AS address_var_date," \
       " ADDRESS_HEADER AS address_var_id,source FROM a5_kit.mysite_bureau_address_segment"
...
data_loan = data_loan.merge(temp1[['deal_id', 'cust_id', 'source', 'loan_id', 'final_written_off_status']], on=[
...
conditions = [(temp['active_status'].notna()), ((temp['active_status'].isna()) & (
    temp['date_closed'].isna()) & (temp['final_written_off_status'] == 0))]
...
data_dpd_final['date_issued'] = pd.to_datetime(
    data_dpd_final['date_issued'], errors='coerce')
data_dpd_final['DPD_month'] = pd.to_datetime(
    data_dpd_final['DPD_month'], errors='coerce')
...
temp['DPD_month_new'] = temp['DPD_month_new'].apply(
...
data_loan = data_loan.merge(
    temp, on=['cust_id', 'source', 'report_id', 'loan_id'], how='left')

# identifying duplicate loans

loan_uniq = data_loan[['deal_id', 'cust_id',
                       'disbursed_amount', 'date_disbursed', 'account_type']]
...
data_loan = data_loan.merge(loan_uniq, on=[
                            'deal_id', 'cust_id', 'disbursed_amount', 'date_disbursed', 'account_type'], how='left')
...
try:
    data_loan['disbursed_amount'] = data_loan['disbursed_amount'].replace(
        to_replace=['None'], value=0)
...
data_loan['tenure_temp'] = data_loan['tenure'].fillna(
    data_loan.groupby('id')['tenure'].transform('max'))
data_loan['tenure_new'] = np.where(((data_loan['tenure_new'].isna()) | (
    data_loan['tenure_new'] == 0)), data_loan['tenure_temp'], data_loan['tenure_new'])
print('tenure_new=', data_loan['tenure_new'])
...
try:
    conditions = [((data_loan['disbursed_amount'] > 0) & (data_loan['tenure'] > 0) & (
        data_loan['emi'] > 0) & ((data_loan['interest_rate'].isna()) | (data_loan['interest_rate'] == 0)))]
except:
    pass

try:
    choices = [npf.rate(data_loan['tenure'], data_loan['emi'], -
                        data_loan['disbursed_amount'], 0)*1200]
except:
    pass

data_loan['interest_rate_new'] = np.select(
    conditions, choices, default=data_loan['interest_rate'])
# print('interest_rate_new =',data_loan['interest_rate_new'])
# imputing tenure max of same group
data_loan['interest_rate'] = data_loan['interest_rate'].replace(0, np.nan)
data_loan['interest_rate_temp'] = data_loan['interest_rate'].fillna(
    data_loan.groupby('id')['interest_rate'].transform('max'))
data_loan['interest_rate_new'] = np.where(((data_loan['interest_rate_new'].isna()) | (
    data_loan['interest_rate_new'] == 0)), data_loan['interest_rate_temp'], data_loan['interest_rate_new'])

# print('interest_rate_new =',data_loan['interest_rate_new'])


# imputation by aavas method
tenure_by_acc_type_loan_amt = pd.read_excel(
    "/Users/Abhishek/Downloads/tenure_by_acc_type_loan_amt_revised.xlsx")
tenure_by_acc_type_for_missing = pd.read_excel(
    "/Users/Abhishek/Downloads/tenure_by_acc_type_for_missing.xlsx")
loan_amt_bins_for_tenure = pd.read_excel(
    "/Users/Abhishek/Downloads/loan_amt_bins_for_tenure_revised.xlsx")
ROI_for_EMI_impute = pd.read_excel(
    "/Users/Abhishek/Downloads/ROI_calculation.xlsx", sheet_name="Sheet_ROI")

crif_cibil_dedup = data_loan.copy()

tenure_by_acc_type_loan_amt['acc_type_new'] = tenure_by_acc_type_loan_amt['acc_type'].astype(
    str).apply(lambda x: re.sub('[\W_]+', '', x).lower())
tenure_by_acc_type_for_missing['acc_type_new'] = tenure_by_acc_type_for_missing['acc_type'].astype(
    str).apply(lambda x: re.sub('[\W_]+', '', x).lower())
loan_amt_bins_for_tenure['acc_type_new'] = loan_amt_bins_for_tenure['acc_type'].astype(
    str).apply(lambda x: re.sub('[\W_]+', '', x).lower())

###############################################################################################################
ROI_for_EMI_impute['last_payment_date_som_new'] = pd.to_datetime(
    ROI_for_EMI_impute['last_payment_date_som_new'], format="%Y-%m-%d")
ROI_for_EMI_impute['acc_type_new'] = ROI_for_EMI_impute['acct_type'].astype(
    str).apply(lambda x: re.sub('[\W_]+', '', x).lower())

# rows 2064
ROI_for_EMI_impute_201604 = ROI_for_EMI_impute[ROI_for_EMI_impute['last_payment_date_som_new'] == pd.to_datetime(
    "2016-04-01")]
ROI_for_EMI_impute_201604 = ROI_for_EMI_impute_201604.rename(
    columns={"ROI_1": "ROI_201604"})
# rows 43

ROI_for_EMI_impute_latest = ROI_for_EMI_impute[ROI_for_EMI_impute['last_payment_date_som_new']
                                               == ROI_for_EMI_impute['last_payment_date_som_new'].max()]
...
############################################################################### tenure#######################################
crif_cibil_dedup = crif_cibil_dedup.rename(
    columns={"acc_type_new": "acc_type_orig"})

conditions = [((~crif_cibil_dedup["acc_type_orig"].isin(["corporatecreditcard", "creditcard", "fleetcard", "kisancreditcard", "loanagainstcard", "loanoncreditcard", "securedcreditcard", "overdraft",
               "primeministerjaandhanyojanaoverdraft", "telcolandline", "telcowireless", "telcobroadband", "autooverdraft"])) & (~crif_cibil_dedup['acc_type_orig'].isin(ROI_for_EMI_impute['acc_type_new'].unique().tolist())))]
choices = ["personalloan"]
crif_cibil_dedup["acc_type_new"] = np.select(
    conditions, choices, default=crif_cibil_dedup['acc_type_orig'])


crif_cibil_dedup_t = pd.merge(
    crif_cibil_dedup, loan_amt_bins_for_tenure.iloc[:, 1:], on='acc_type_new', how='left')
...
try:
    ...
    pass
try:
    choices = [crif_cibil_dedup_1['mode_tenure'],
               crif_cibil_dedup_1['mode_tenure_acct_type'], crif_cibil_dedup_1['tenure_new']]
except:
    pass

try:
    crif_cibil_dedup_1['tenure_new'] = np.select(
...
try:
    choices = [crif_cibil_dedup_4['ROI_201604'],
               crif_cibil_dedup_4['ROI_latest'], crif_cibil_dedup_4['ROI_1']]
...
try:
    ...
    pass
try:
    crif_cibil_dedup_4['tenure_new'] = crif_cibil_dedup_4['tenure_new'].astype(
        float)
except:
    pass
try:
    crif_cibil_dedup_4['t'] = (
        1+crif_cibil_dedup_4['r'])**(crif_cibil_dedup_4['tenure_new'])
except:
    pass
try:
    crif_cibil_dedup_4['emi_temp'] = (crif_cibil_dedup_4['disbursed_amount'] *
                                      crif_cibil_dedup_4['r']*crif_cibil_dedup_4['t'])/(crif_cibil_dedup_4['t']-1)
except:
    pass

try:
    crif_cibil_dedup_4['emi_new'] = np.where(((crif_cibil_dedup_4['emi'].isna()) | (
        crif_cibil_dedup_4['emi'] == 0)), crif_cibil_dedup_4['emi_temp'], crif_cibil_dedup_4['emi'])
except:
    pass
try:
...
first = data_loan_1[(((data_loan_1['disbursed_amount'].isna()) | (
    data_loan_1['disbursed_amount'] == 0)) & ((data_loan_1['emi'].isna()) | (data_loan_1['emi'] == 0)))]
first['extra_column'] = "emi and disbursed amount both are zero"

data_loan_1 = data_loan_1[~data_loan_1['serial_number'].isin(
    first['serial_number'].unique().tolist())]
# unique loans
cnt_source_1 = pysqldf(
    'select id, count(source) as cnt_source_1 from data_loan_1 group by id')

dedup_data_2 = pysqldf(
    'select a.*, b.cnt_source_1 from data_loan_1 a left join cnt_source_1 b on a.id=b.id')
dedup_dataset1 = dedup_data_2[dedup_data_2['cnt_source_1'] == 1]
dedup_dataset2 = dedup_data_2[dedup_data_2['cnt_source_1'] > 1]
# based on ownership
dedup_dataset2a = pysqldf(
    'select * from dedup_dataset2 where ownership is null or ownership not in ("Supl Card Holder","Guarantor","Joint")')

dedup_dataset2a_remain = pysqldf(
    'select * from dedup_dataset2 where id not in (select distinct id from dedup_dataset2a)')
cnt_source_2a = pysqldf(
    'select id, count(source) as cnt_source_2 from dedup_dataset2a group by id')

dedup_dataset2a = pysqldf(
    'select a.*, b.cnt_source_2 from dedup_dataset2a a left join cnt_source_2a b on a.id=b.id')
...
dedup_dataset3b_1 = pysqldf(
    'select * from dedup_dataset3b where date_reported=max_date_reported')
dedup_dataset3b_1_remain = pysqldf(
    'select * from dedup_dataset3b where id not in (select distinct id from dedup_dataset3b_1)')
cnt_source_3b = pysqldf(
    'select id, count(source) as cnt_source_3 from dedup_dataset3b_1 group by id')
dedup_dataset3b_1 = pysqldf(
    'select a.*, b.cnt_source_3 from dedup_dataset3b_1 a left join cnt_source_3b b on a.id=b.id')
...
# last DPD
temp = dedup_dataset4b.groupby('id')['DPD_month'].max(
).reset_index(name="most_recent_dpd_month")
dedup_dataset4b = dedup_dataset4b.merge(temp, on='id', how='left')

dedup_dataset4b_1 = pysqldf(
    'select * from dedup_dataset4b where DPD_month=most_recent_dpd_month')

dedup_dataset4b_1_remain = pysqldf(
    'select * from dedup_dataset4b where id not in (select distinct id from dedup_dataset4b_1)')
cnt_source_4b = pysqldf(
    'select id, count(source) as cnt_source_4 from dedup_dataset4b_1 group by id')
dedup_dataset4b_1 = pysqldf(
    'select a.*, b.cnt_source_4 from dedup_dataset4b_1 a left join cnt_source_4b b on a.id=b.id')
...
dedup_dataset5b_1_remain = pysqldf(
    'select * from dedup_dataset5b where id not in (select distinct id from dedup_dataset5b_1)')
cnt_source_5b = pysqldf(
    'select id, count(source) as cnt_source_5 from dedup_dataset5b_1 group by id')
dedup_dataset5b_1 = pysqldf(
    'select a.*, b.cnt_source_5 from dedup_dataset5b_1 a left join cnt_source_5b b on a.id=b.id')
...
dedup_dataset6b_1_remain = pysqldf(
    'select * from dedup_dataset6b where id not in (select distinct id from dedup_dataset6b_1)')
cnt_source_6b = pysqldf(
    'select id, count(source) as cnt_source_6 from dedup_dataset6b_1 group by id')
dedup_dataset6b_1 = pysqldf(
    'select a.*, b.cnt_source_6 from dedup_dataset6b_1 a left join cnt_source_6b b on a.id=b.id')
...
dedup_dataset8a = pysqldf(
    'select * from dedup_dataset7b order by id, source_map')
...
dedup_dataset_remain_final = pysqldf(
    'select * from dedup_dataset_remain order by id, source_map')
dedup_dataset_remain_final = dedup_dataset_remain_final.groupby(
    'id').nth(0).reset_index()
...
remaining = data_loan[~data_loan['serial_number'].isin(
    data_loan_final['serial_number'].unique().tolist())]
...
cols = ['Tenure', 'Tenure_new', 'ROI', 'ROI_new', 'ROI_edited',
        'EMI_new', 'EMI_edited', 'DPD', 'DPD_month_new']
try:
...
final_data = final_data.sort_values(
    ['Disbursal_date', 'Loan_type', 'Disbursed_amount'], ascending=[False, True, True])
final_data['index'] = list(range(1, len(final_data)+1))
final_data['DPD'] = final_data['DPD'].apply(
    lambda x: 'NULL' if (x == 'nan' or x == None) else x)
final_data.to_csv("/Users/Abhishek/Downloads/" +
                  cid + "_bureau.csv", index=False)
