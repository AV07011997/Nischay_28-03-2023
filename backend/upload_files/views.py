...
#
# def upload_statements(request, text):
#     queryset = upload_file_details.objects.all().annotate(Count("lead_id")).filter(lead_id=text).values("lead_id", "name")
#     # queryset = upload_file_details.objects.all().values("lead_id", "name")
#
#     data = pd.DataFrame(list(queryset))
#
#     queryset1 = los_did_cid_generation.objects.all().filter(lead_id=text).values("lead_id", "name")
